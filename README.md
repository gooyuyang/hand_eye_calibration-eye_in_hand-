...
## 程序介绍
本程序是在睿尔曼机械臂手眼标定的程序的基础上进行修改的（https://github.com/RealManRobot/hand_eye_calibration）

本程序只涉及到手眼标定里面的Eye-in-Hand，因此也只对源程序compute_in_hand.py进行修改得到newcompute_in_hand.py

原来的compute_in_hand.py有一个致命缺陷：

原程序只要图片存在并且检测到了棋盘格角点，就加入计算；

但如果某些图片检测失败，就会出现图片与对应的机械臂姿态不对应的结果，例如：

如果 3.jpg 检测失败，原程序可能会把 4.jpg 的图像结果错误地对应到 机械臂姿态poses.txt 第 3 行，导致图像和机械臂位姿错位，那么最终结果也会出现较大的计算误差。

由于本人的棋盘格存在一定的损伤，，会导致部分图片检测失误，那么当使用源程序进行标定时，就会出现特别大的误差

于是本人对源程序进行了修改，从“默认所有图片都有效、编号必须连续”改成了“自动筛选有效图片，并让图片编号和机械臂位姿严格对应”。

特别感谢大佬Daniel-rmc（https://github.com/Daniel-rmc）

在issue里面指出标定程序的问题所在https://github.com/RealManRobot/hand_eye_calibration/issues/14

结合大佬的修改意见，得到了新的程序**newcompute_in_hand.py**，经过我的损伤棋盘格验证，标定无误
